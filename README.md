...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create chatmate-f9d4
  fly apps create chatmate-f9d4-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app chatmate-f9d4
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app chatmate-f9d4-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'chatmate-f9d4-staging' does not match app name in config file 'chatmate-f9d4'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name chatmate-f9d4-db
  fly postgres attach --app chatmate-f9d4 chatmate-f9d4-db

  fly postgres create --name chatmate-f9d4-staging-db
  fly postgres attach --app chatmate-f9d4-staging chatmate-f9d4-staging-db
  ```
