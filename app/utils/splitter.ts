...
export abstract class TextSplitter implements TextSplitterParams {
  ...
  private async createDocumentsFromText(
    text: string,
    metadata: Metadata,
  ): Promise<Document[]> {
    ...
    for (const chunk of await this.splitText(text)) {
      const intermediateNewLines = this.getIntermediateNewLines(
        text,
        prevChunk!,
        chunk,
      );
      ...
      const updatedMetadata = this.getLoc(
        metadata,
        lineCounterIndex,
      ...
      documents.push(
        new Document({ pageContent: chunk, metadata: updatedMetadata }),
      );
  ...
  async createDocuments(
    ...
    const documentPromises = texts.map((text, i) =>
      this.createDocumentsFromText(text, metadatas[i]),
    );
    ...
    for (const d of splits) {
      ...
      if (total + len >= this.chunkSize) {
        ...
        if (currentDoc.length > 0) {
          const doc = this.createDocAndAdjustCurrentDoc(
            currentDoc,
            separator,
            total,
            len,
